...
image = next(iter(cars_train_dl))[0]
num_images = 10
stepsize = int(T/num_images)
fig, axes = plt.subplots(1, num_images, figsize=(15, 15))
...
for idx in range(0, T, stepsize):
    ...
    plt_img = show_StanfordCars_tensor_image(img)
    axes[int(idx/stepsize)].set_title(f"t={int(idx)}")
    axes[int(idx/stepsize)].imshow(plt_img)

# save figs
plt.tight_layout()
plt.savefig(f"results/valid_cars.png")
plt.clf()

#########
# MODEL #
#########
model = SimpleUnet(img_channel=3)   
...
for epoch in range(epochs):
    for step, batch in enumerate(cars_train_dl):
      # zero_grad
      optimizer.zero_grad()
      ...
      if epoch % 50 == 0 and step == 0:
        # save ckpt
        torch.save(model.state_dict(), f"results/model_stanford_cars.pt")
        ...
        # generate imgs
        sample_plot_image(
            IMG_SIZE=StanfordCars_IMG_SIZE,
            device=device,
...
results_df.to_csv("results/results_stanford_cars.csv", index=False)